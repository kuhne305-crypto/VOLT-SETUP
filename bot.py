...
class VerifyView(discord.ui.View):
    ...
    async def verify(self, interaction: discord.Interaction, button: discord.ui.Button):
        role = branding.resolve_role_by_base(interaction.guild, self.role_name)
        if role is None:
            for base, emoji, color, hoist in branding.ROLE_CONFIG:
                if base == self.role_name:
                    role = await interaction.guild.create_role(
                        name=branding.full_role_name(base), color=color, hoist=hoist, reason="Verify-System"
                    )
                    break
            else:
...
async def ensure_log_channel(guild: discord.Guild) -> discord.TextChannel:
    full_name = branding.channel_name("🔐", "admin-logs")
    channel = discord.utils.get(guild.text_channels, name=full_name)
    if channel is None:
        overwrites = {guild.default_role: discord.PermissionOverwrite(view_channel=False)}
        channel = await guild.create_text_channel(full_name, overwrites=overwrites, reason="Setup: Log-Kanal")
    return channel
...
async def ensure_roles(guild: discord.Guild) -> dict:
    """Legt die VOLT-Rollen mit Farbe + Emoji an (oder aktualisiert sie) und
    setzt danach ihre Reihenfolge, damit die Hierarchie auch im Server
    (Mitgliederliste, Rollen-Rangfolge) sichtbar ist: Admin oben, Verified
    knapp über @everyone."""
    roles = {}
    for base, emoji, color, hoist in branding.ROLE_CONFIG:
        full_name = branding.full_role_name(base)
        existing = branding.resolve_role_by_base(guild, base)
        if existing:
            await existing.edit(name=full_name, color=color, hoist=hoist, reason="VOLT Setup: Rolle aktualisiert")
            roles[base] = existing
        else:
            roles[base] = await guild.create_role(
                name=full_name, color=color, hoist=hoist, reason="VOLT Setup"
            )

    ordered = [roles[base] for base, *_ in branding.ROLE_CONFIG]
    positions = {role: len(ordered) - i for i, role in enumerate(ordered)}
    try:
        await guild.edit_role_positions(positions=positions, reason="VOLT Setup: Rollen-Reihenfolge")
    except discord.HTTPException:
        log.warning("Konnte Rollen-Reihenfolge nicht setzen (Bot-Rolle evtl. zu niedrig in der Hierarchie).")
...
async def build_server_structure(guild: discord.Guild) -> discord.TextChannel:
    ...
    rules_channel = await ensure_channel("📜", "regeln", welcome_cat, overwrites=read_only_overwrites)
    ...
    await ensure_channel("🗂️", "ticket-logs", ticket_cat, overwrites=staff_only_overwrites)
    ...
    if rules_channel.last_message_id is None:
        embed = discord.Embed(
            title="📜 Server-Regeln",
            description=(
                "Mit dem Betreten dieses Servers akzeptierst du die folgenden Regeln. "
                "Verstöße können je nach Schwere zu Verwarnung, Timeout, Kick oder Bann führen."
            ),
            color=VOLT_RED,
        )
        embed.add_field(
            name="🤝 Allgemeines Verhalten",
            value=(
                "• Respektvoller Umgang - kein Spam, kein Beleidigen, keine Diskriminierung\n"
        ...
        embed.add_field(
            name="📢 Werbung & Spam",
            value=(
                "• Eigenwerbung, Fremdlinks/-server und DM-Werbung ohne Absprache sind verboten\n"
                "• Kein Massenerwähnen (@everyone/@here) durch Nicht-Staff"
            ),
            inline=False,
        )
        embed.add_field(
            name="🛒 Ticket- & Bestellsystem",
            value=(
                "• Tickets nur für echte Anliegen öffnen, kein Missbrauch\n"
                "• Preise & Leistungen stehen in #preisliste - verbindliche Absprachen laufen über Tickets\n"
                "• Zahlungs-/Lieferbetrug führt zum sofortigen Bann"
            ),
            inline=False,
        )
        embed.add_field(
            name="🔐 Sicherheit",
            value=(
                "• Keine Nuke-/Raid-Versuche, Selfbots oder Exploits\n"
                "• Verdächtige Accounts/Links bitte dem Team melden"
            ),
            inline=False,
        )
        embed.add_field(
            name="⚖️ Konsequenzen",
            value="Das Team entscheidet im Einzelfall über die Maßnahme - die Entscheidung ist verbindlich.",
            inline=False,
        )
    ...
    if welcome_channel.last_message_id is None:
        embed = discord.Embed(
            ...
            description=(
                "Discord Solutions - Server Protection & Full Control.\n"
                "Lies dir kurz die Regeln im Regeln-Kanal durch und schau danach beim Verify-Kanal vorbei, "
...
async def add_category(interaction: discord.Interaction, name: str, staff_only: bool = False):
    ...
    overwrites = None
    if staff_only:
        staff_roles = [branding.resolve_role_by_base(guild, n) for n in STAFF_ROLE_NAMES]
